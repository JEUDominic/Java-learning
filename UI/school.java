...
public class school extends JFrame implements ActionListener{
		...
		public void actionPerformed(ActionEvent evt) {
			...
			if (actionCommand.equals("back")){
				...
				int id = Validation.schoolCheck(s);
				int checkValue = 0;
				if(id > 0){
					checkValue = Insert.alumni(new Alumni(alumni.getStudentID(),alumni.getName(),alumni.getSex(),alumni.getMajor(),1,alumni.getGraYear(),id));
					if(checkValue == id){
						this.dispose();
						new chenggong();
					}
					...
					this.dispose();
					new Schoolinfo(alumni,tf.getText());
				}
			}
		}
	}